...
初回のみ、別ターミナルで `codex --login` を実行してサインインしてください。
...
| `first_setup.sh` | tmux、Node.js、Codex CLI をインストール | 初回のみ |
...
IDE拡張版のCodexはスクショを貼り付けて事象を説明できます。このCLIシステムでも同等の機能を実現：
...
`.codex/skills/` 配下のスキルはリポジトリにコミットしない設計。理由：
- 各ユーザの業務・ワークフローは異なる
- 汎用的なスキルを押し付けるのではなく、ユーザが自分に必要なスキルを育てていく

スキル保存先：
- 個人共通: `~/.codex/skills/`
- プロジェクト専用: `./.codex/skills/`
...
MCPはCodexに外部ツールやドキュメントへのアクセスを提供します。CLIとIDE拡張の両方で使え、設定は共有されます。

### 設定ファイル

MCP設定は `~/.codex/config.toml` に保存されます。プロジェクト単位にしたい場合は `./.codex/config.toml` を使います。
...
# 例: Sequential Thinking - 複雑な問題を段階的に思考
codex mcp add sequential-thinking -- npx -y @modelcontextprotocol/server-sequential-thinking

# 例: Memory - セッション間の長期記憶（推奨！）
...
- CLI: `codex mcp --help` で利用可能なコマンドを確認
- TUI: `codex` 起動後に `/mcp` で有効なサーバ一覧を確認
...
│
├── .codex/
│   └── skills/               # Codexスキル（ローカル）
│
├── memory/                   # Memory MCP保存場所
├── dashboard.md              # リアルタイム状況一覧
└── CLAUDE.md                 # Codex用プロジェクトコンテキスト（ファイル名は互換のため維持）
```
...
<details>
<summary><b>MCPが見えない？</b></summary>

以下を確認してください：

- 設定ファイル：`~/.codex/config.toml` または `./.codex/config.toml`
- TUI：`codex` 起動後に `/mcp` で有効なサーバ一覧を確認
- CLI：`codex mcp --help` でコマンド一覧を確認

</details>

<details>
<summary><b>承認が止まる？</b></summary>

Codexの承認モードを確認してください：

- 一時的に切り替える場合：`codex --approval-mode on-failure "..."`  
- 恒久設定する場合：`~/.codex/config.toml` の `approval_policy` を設定
