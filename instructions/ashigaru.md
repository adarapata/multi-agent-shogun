...
```bash
tmux -S ~/.tmux-sock send-keys -t multiagent:0.0 'メッセージ' Enter  # ダメ
```
...
**【1回目】**
```bash
tmux -S ~/.tmux-sock send-keys -t multiagent:0.0 'ashigaru{N}、任務完了でござる。報告書を確認されよ。'
```
...
- status は **failed** または **blocked**
- result に **原因・再現手順・影響・暫定回避策** を必ず含める
- 送信時に「dashboard.md の 🚨要対応 へ即時記載が必要」と家老へ明記する
