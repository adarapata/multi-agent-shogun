...
# ═══════════════════════════════════════════════════════════════════════════════
# STEP 6: Codex CLI 起動（--setup-only でスキップ）
# ═══════════════════════════════════════════════════════════════════════════════
if [ "$SETUP_ONLY" = false ]; then
    log_war "👑 全軍に Codex CLI を召喚中..."
    log_info "🔑 初回は別ターミナルで 'codex --login' を実行してログインしてください"

    # 将軍
    tmux send-keys -t shogun "codex"
    ...
    # 家老 + 足軽（9ペイン）
    for i in {0..8}; do
        tmux send-keys -t "multiagent:0.$i" "codex"
        tmux send-keys -t "multiagent:0.$i" Enter
    done
    log_info "  └─ 家老・足軽、召喚完了"

    log_success "✅ 全軍 Codex CLI 起動完了"
    echo ""
...
if [ "$SETUP_ONLY" = true ]; then
    ...
    echo "  ┌──────────────────────────────────────────────────────────┐"
    echo "  │  # 将軍を召喚                                            │"
    echo "  │  tmux send-keys -t shogun 'codex' Enter                   │"
    echo "  │                                                          │"
    echo "  │  # 家老・足軽を一斉召喚                                   │"
